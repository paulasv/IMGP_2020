...
# Plots of status in time (Extended materia Fig)##############Reviewer suggestion########
png("new_fig1.png", width = 3200, height = 2500, res = 300)
data_F %>%
  filter(!is.na(FMSY.x), !is.na(FMSY.y))%>%
  filter(change.x ==TRUE)%>%
  filter(!newname.x %in% nephrops)%>%
...
png("new_fig2.png", width = 3800, height = 2500, res = 300)
data_B %>%
  filter(!is.na(MSYBtrigger.x), !is.na(MSYBtrigger.y))%>%
  filter(change.x ==TRUE)%>%
  filter(!newname.x %in% nephrops)%>%
...
### new plot##########################################################################################################
png("new_fig5.png", width = 2800, height = 1500, res = 300)
ggarrange(p1, p2,  nrow = 2, heights = c(2,1))  
dev.off()
######################################################################################
...
ggplot(aes(delta, y)) +
  ...
  geom_line(aes(delta, 1/delta-1), linetype = 2)+ # the expected relationship
  geom_point(size = 0.5)+
...
# expected change in the ration with a change in delta
ebe <- datap %>%
  ggplot(aes(delta_b, y_b)) +
  geom_vline(xintercept = 1, linetype = 3, color = "lightgrey")+
  geom_hline(yintercept = 0, linetype = 3, color = "lightgrey")+
  #geom_smooth(method = "loess",linetype = 1, color = "darkgrey")+
  geom_line(aes(delta_b, 1/delta_b-1), linetype = 2)+# the expected relationship
  geom_point(size = 0.5)+
  scale_colour_viridis_d()+
  scale_y_continuous(labels = scales::percent)+
  geom_text(aes(label = lab), color = "black", vjust = -1, hjust = .5, check_overlap = TRUE, alpha = 0.9, size = 2.5)+
...
### new plot##########################################################################################################
png("new_fig6.png", width = 2800, height = 1500, res = 300)
ggarrange(efe, ebe, nrow = 1, common.legend = TRUE, legend = "right")
dev.off()
######################################################################################
