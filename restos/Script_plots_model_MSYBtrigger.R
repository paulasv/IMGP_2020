...
data <- read.csv("covariates_B_rev.csv")
dos <- data %>%
...
B_rev <- ggplot(data3_melt, aes(y=factor(revision_type, level=as.character(rev(frec$revision))) , x=factor(variable, level=as.character(frec$revision)), fill = value)) +
  geom_raster() + 
...
data <- datab %>%
  ...
  mutate(former = mean(delta_b, na.rm = TRUE))%>%
  ungroup()%>%
  group_by(Current_MSYBtrigger)%>%
  mutate(current =mean(delta_b, na.rm = TRUE))%>%
  ungroup()%>%
  group_by(Former_MSYBtrigger_, Current_MSYBtrigger)%>%
  summarize(n = n(), 
            meand = mean(delta_b, na.rm = TRUE), 
            refy = -mean(former, na.rm = TRUE),
            refx = mean(current, na.rm = TRUE))%>%
  filter(Former_MSYBtrigger_ != 0 )%>%
  ungroup()%>%
  mutate(refy = c(1,1,6,4.5,3,-1))%>%
  mutate(refx = c(4.5,-1,6,6,4.5,4.5))

con <- data%>%
  ggplot(aes(x =1 , xend = 2, y = refy, yend = refx, colour = meand, size =n),show.legend = T)+
  geom_segment()+
  geom_point(aes(x =1 , y = refy), size = 15, colour = "papayawhip", alpha = .7)+
  geom_point(aes(x = 2, y = refx), size = 15, colour = "papayawhip", alpha = .7)+
  scale_colour_gradient2(low = "#009E73", midpoint = 1,mid =  "lemonchiffon1",  high = "#E69F00" ,limits = c(0, 2) ,  na.value = "#E69F00" )+
  geom_text(label=data$Former_MSYBtrigger_, x=rep(1, NROW(data)), colour = "black",  check_overlap = T, nudge_x =1, size=3)+
  geom_text(label=data$Current_MSYBtrigger, y = data$refx, x=rep(2, NROW(data)), check_overlap = T,colour = "black", nudge_x =1, size=3)+
...
ggsave(con, filename = "SI10.png", units = "cm", width = 25, height = 15, dpi = 800)
